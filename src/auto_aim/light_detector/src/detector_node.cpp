...
namespace rm_auto_aim_dart
{
    ...
    std::vector<Detector::Light> LightDetectorNode::detectLights(const sensor_msgs::msg::Image::ConstSharedPtr &img_msg, cv::Mat &img)
    {
        // convert ROS image to cv::Mat
        img = cv_bridge::toCvShare(img_msg, "bgr8")->image;
        auto lights = detector_->find_lights(img, detector_->binary(img));
        // 计算延迟
        ...
        if (debug_)
        {
            binary_img_pub_.publish(
                cv_bridge::CvImage(img_msg->header, "mono8", detector_->binary_image).toImageMsg());
